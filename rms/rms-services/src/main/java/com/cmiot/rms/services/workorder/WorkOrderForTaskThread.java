 ...
 */
public class WorkOrderForTaskThread implements Runnable,Serializable {

    private static final long serialVersionUID = 1L;

    private Logger logger = LoggerFactory.getLogger(WorkOrderForTaskThread.class);

    private  List<GatewayBusiness> gatewayBusinessList;
    ...
    public WorkOrderForTaskThread(List<GatewayBusiness> gatewayBusinessList, GatewayInfo gatewayInfo){
        this.gatewayBusinessList = gatewayBusinessList;
    ...
    public void run() {
        try {
        	logger.info("start invoke excuteWorkOrder,循环任务触发");
            
        	if (gatewayBusinessList==null||gatewayBusinessList.isEmpty()) {
        		logger.info("gatewayBusinessList为空");
        		return;
			}
            ...
            if (gatewayBusinessList.size()>1) {
            	for(int m = 0;m<gatewayBusinessList.size();m++){
                	//宽带新装
                    if(gatewayBusinessList.get(m).getBusinessCode().startsWith("wband") &&"1".equals(gatewayBusinessList.get(m).getBusinessType())){
                        wbandGatewayBusiness = gatewayBusinessList.get(m);
                        wbandLocation = m;
                        isHaveWband = true;
                        break;
                    }
                ...
                if(isHaveWband) {
                    gatewayBusinessList.remove(wbandLocation);
                    gatewayBusinessList.add(0, wbandGatewayBusiness);
                }
			}
            
            //遍历执行工单
            //下发业务条数
            boolean sendNumResult = setOneCommand(gatewayInfo.getGatewayMacaddress(), "InternetGatewayDevice.X_CMCC_UserInfo.ServiceNum", String.valueOf(gatewayBusinessList.size()));
            if (!sendNumResult) {
            	logger.info("下发工单条数到网关password失败:{}", gatewayInfo.getGatewayPassword());
            	return;
			}
            for (GatewayBusiness gb : gatewayBusinessList) {
                //查询模板信息
